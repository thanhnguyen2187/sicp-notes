; Exercise 1.17
;
; The exponentation algorithms in this section are based performing
; exponentation by means of repeated multiplication. In a similar way, one can
; perform integer multiplication by means of repeated addition. The following
; multiplication procedure (in which it is assumed that our language can only
; add, not multiply) is analogous to the `expt` procedure:
;
; (define (* a b)
;   (if (= b 0)
;       0
;       (+ a (* a (- b 1)))))
;
; This algorithm takes a number of steps that is linear in b. Now suppose
; we include, together with addition, operation `double`, which double an
; integer, and `halve`, which divides an (even) integer by 2. Using these,
; design a multiplication procedure analogous to `fast-expt` that uses a
; logarithmic number of steps.

#lang sicp

(define (is-even? x)
  (= (remainder x 2) 0))

(define (double x) (+ x x))
(define (halve x) (/ x 2))

(define (multiply a b)
  (cond ((= b 0) 0)
        ((= b 1) a)
        ((is-even? b) (multiply (double a) (halve b)))
        (else (+ a (multiply (double a) (halve (- b 1)))))))

(multiply 6 7)
(multiply 7 8)
