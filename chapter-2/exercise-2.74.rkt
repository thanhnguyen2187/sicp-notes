...
;
; Instatiable's president, in her first attempt to exploit the ability of the
...
; As an example, suppose that each divison's personnel records consist of a
...
; division) that contains information keyed under indentifiers such as `address`
; and `salary`. In particular:
;
; a. Implement for head quarters a `get-record` procedure that retrieves a
...
; must be supplied?
; b. Implement for headquarters `get-salary` procedure that returns the salary
