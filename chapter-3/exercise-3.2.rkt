...
; keeps track of the number of times it has been called by maintaining an
; internal counter.
...
; counter to zero.
; - For any other input, `mf` returns the result of calling f on that input and
; increments of the ocunter.
;
  ...
  (let ((counter 0))
    (define (mf input)
      (cond ((eq? input 'how-many-calls?) counter)
            ((eq? input 'reset-count) (begin (set! counter 0)
                                              0))
            (else
